...
If viewing this from ProPack make sure to checkout the pro_pack/devkit folder
...
class MClassNodeUI(object):
    ...
    def _showUI(self):
        ...
        cmds.menuItem(divider=True)
        cmds.menuItem(l=LANGUAGE_MAP._MetaNodeUI_.pro_addchild_subsystem, command=partial(self.__uiCB_connectChildMetaNode, '_MetaRigSubSystem'))
        cmds.menuItem(l=LANGUAGE_MAP._MetaNodeUI_.pro_addchild_support, command=partial(self.__uiCB_connectChildMetaNode, '_MetaRigSupport'))
        cmds.menuItem(divider=True)
...
#                     return msgLinks[0]  # MetaClass(msgLinks[0])
    ...
    def __getattribute__(self, attr):
        ...
        try:
            ...
            objectattr = True
        except:  # we don't won't this to raise an error as we need it to pass into the Maya block
            if logging_is_debug():
    ...
    def hasAttr(self, attr):
        ...
        if self.isValidMObject():
            try:
                ...
                # self._MFnDependencyNode.findAlias(attr, self._MObject)  do we include aliasAttr check?
            except:
...
#         if hasattr(attr, '__iter__'):
    ...
    def getEnumAttr_as_string(self, attr):
        '''
        from a given enum attr on this mNode, return the current enum value as a string
        ''' 
        if self.attrType(attr) == 'enum':
            enums = cmds.attributeQuery(attr, node=self.mNode, listEnum=True)[0].split(':')
            return enums[getattr(self, attr)]
...
#     @r9General.Timer
    ...
    @staticmethod
    def getNodeConnectionMetaDataMap(node, mTypes=[], mNodes=[]):
        '''
        ...
        :param mTypes: if given this is a list of specific mTypes that the connected mNode must be
        :param mNodes: if given this is a list of specific mNodes themselves that we clamp the return too. This allows us to
            clamp the return to a specific mRig system IF we need a controller wired to multiple mRig systems, such as weapons
        :return: mNode_data={} which is directly pushed into the PoseFile under the [metaData] key
        ...
        for con in connections[1::2]:
            ...
            if isMetaNode(data[0], mTypes=mTypes):
                ...
                try:
                    ...
                    mNode_data['metaNodeID'] = node.split(':')[-1].split('|')[-1]
                return mNode_data
            elif mTypes:
                continue
...
class MetaRig(MetaClass):
    ...
    in a seemless manner and bind some core functionality.
    '''
    ...
    def __bindData__(self):
        '''
        additional binding process used in subclassing to ensure that given attrs exist
        on instantiation. 
        
        .. note::
            All attrs added here will propagate to all ProPack metanodes by design
        '''
        ...
        self.addAttr('scaleSystem', attrType='messageSimple')   # Red9 PuppetRig scale hook
        self.addAttr('timecode_node', attrType='messageSimple')  # wire that denotes the node to use for the timecode attrs in pro
    ...
    def ctrl_offset(self):
        ...
        .. note::
            this new Offset controller is treated as a root in all of the tools, so if the "incRoot" 
            flag is off in the anim tools then this node will also be ignored along with the ctrl_main
        '''
        if self.hasAttr('%s_Offset' % self.CTRL_Prefix):
            try:
                return getattr(self, '%s_Offset' % self.CTRL_Prefix)[0]
            except:
                log.warning('CTRL_Offset was not connected')

        log.info('mRig has no "CTRL_Offset" bound')    
...
#         # pro hook for non-PuppetRigs
#         elif self.hasAttr('exportTag') and self.exportTag:
#             # no wire, we may be dealing with an SRC Ext node
#             if self.exportTag and self.exportTag.exportRoot:
#                 return self.exportTag.exportRoot
        return None
...
#         return ctrls
    ...
    def poseClipboard_valid(self):
        '''
        check the OS Clipboard for a compatible poseCache object. This is used to verify
        ...
        if type(datamap) == str:
            try:
                ...
                pass
#                 log.warning('OS Clipboard not compatible with poseCache systems, regressing to internal mRig Cache if built')

    @nodeLockManager
    def poseCacheStore(self, attr=None, filepath=None, incRoots=True, storeThumbnail=False, to_clipboard=False, *args, **kws):
        '''
    ...
    def poseCacheLoad(self, nodes=None, attr=None, filepath=None, incRoots=True, relativePose=False, relativeRots='projected',
                      relativeTrans='projected', maintainSpaces=False, skipAttrs=[], from_clipboard=False, *args, **kws):
        '''
...
#         import Red9_CoreUtils as r9Core  # lazy loaded
    ...
    def cutKeys(self, nodes=[], reset=True, walk=True, verbose=False, selected=False):
        '''
        ...
        :param walk: True by default, walk the mRig structures to find all children
        :param verbose: raise the confirm dialog
        :param selected: this is primary for the DAG menus in ProPack, rather than pass in nodes we
            find all children of this mRig and compare the selected nodes against that,
            only cutting those nodes that are part of this mRig
        '''
        if verbose:
        ...
        if not nodes:
            nodes = self.getChildren(walk=walk)
            if selected:
                nodes = [n for n in cmds.ls(sl=True, l=True) if n in nodes]
    ...
    def saveAnimation(self, filepath=None, incRoots=True, useFilter=True, timerange=(),
        ...
        :param incRoots: do we include the root node in the save, in metaRig case this is ctrl_main
        ...
        if r9Setup.has_pro_pack():
            ...
            self.animCache = r9panim_map.AnimMap(**kws)
    ...
    def animMap_preprocess(self, feedback, *args, **kws):
        '''
        : PRO_PACK :
            Added to be Overloaded at the class level!

        Call passed into the animMap class and run BEFORE the r9Anim file is loaded
        on the MetaRig, this allows you to add functionality to the base load call to
        extract extra data from the animMap and act upon it. This allows us to act on the
        animMap stored on the class object and rebuild data from the infoDict if required.
        '''
    ...
    def loadAnimation(self, filepath=None, incRoots=True, useFilter=True, loadAsStored=True, loadFromFrm=0, loadFromTimecode=False,
        ...
        :param maintain_parents: Only supported with mRig load data. This preserves any parentSwitching mismatches between the stored data
            ...
            in the filterPriority list as those are the ones that get recalculated in worldSpace after the data load.
...
#             self.animCache.api=False
            ...
            try:
                ...
                feedback = self.animCache.loadAnim(nodes=rootNodes,
                                                   ...
                                                   maintain_parents=maintain_parents,
                                                   **kws)
                # =========================================================
...
class MetaRigSubSystem(MetaRig):
    ...
    def build_class(self):
        '''
        get the internal build class for this compound
        '''
        if self.hasAttr('buildFlags') and 'build_class' in self.buildFlags:
            return self.buildFlags['build_class']
...
#     
#         # Source Mel Files
#         cmds.eval('source "'+MAYA_LOCATION+'/scripts/others/hikGlobalUtils.mel"')
#         cmds.eval('source "'+MAYA_LOCATION+'/scripts/others/hikCharacterControlsUI.mel"')
#         cmds.eval('source "'+MAYA_LOCATION+'/scripts/others/hikDefinitionOperations.mel"')
#         cmds.eval('source "'+MAYA_LOCATION+'/scripts/others/CharacterPipeline.mel"')
#         cmds.eval('source "'+MAYA_LOCATION+'/scripts/others/characterSelector.mel"')
    ...
    def isLocked(self):
        '''
        return True if the HIK characterization is locked
        '''
        return self.InputCharacterizationLock
    ...
    def setascurrentcharacter(self, openui=True):
        '''
    ...
    def checkcharacterization(self, openui=True):
        ...
        :param openui: if True we open the UI to show the status, else we just return the status
        :return: True if valid, False if invalid
        '''
        self.setascurrentcharacter(openui=openui)
        if openui:
            self.openui()
...
class MetaHUDNode(MetaClass):
    ...
    def resetSlider(self, slider, attr, *args):
        '''
        ...
        value = 0
        try:
            _value = cmds.addAttr(attr, q=True, dv=True)
            if _value:
                value = _value
        except:
            pass
